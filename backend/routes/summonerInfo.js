...
async function getSummonerId(name, reg) {
  let summonerId;
  await fetch(`https://${reg}.api.riotgames.com/lol/summoner/v4/summoners/by-name/${name}?api_key=${API_KEY}`)
    .then(res => res.json())
    .then(data => {
        summonerId = data.id;
    })
...
async function getSummonerInfo(name, reg) {
  ...
  let summonerInfo;
  await fetch(`https://${reg}.api.riotgames.com/lol/league/v4/entries/by-summoner/${id}?api_key=${API_KEY}`)
    .then(res => res.json())
    .then(data => {
        summonerInfo = data;
    })
